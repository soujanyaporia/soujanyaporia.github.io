...
export function Coverage(){
 ...
 const withLesson=rows.filter(r=>r.lessons.length).length,withPractice=rows.filter(r=>r.activities.length).length;
 ...
 return <main className="teach coverage"><a className="back-link" href="#/teach">← Back to Learn</a><p className="teach-eyebrow">Curriculum coverage</p><h1>What is built, and what is not.</h1>
  ...
  <p className="cov-summary" role="status"><strong>{withLesson} of {rows.length}</strong> objectives have a Learn lesson · <strong>{withPractice} of {rows.length}</strong> have practice activities · <strong>{rows.length-withPractice}</strong> are mapped only.</p>
  <div className="table-scroll"><table className="cov-table"><thead><tr><th>Objective</th><th>Learn</th><th>Guided</th><th>Practice</th><th>Visual</th><th>Word</th><th>Mastery</th></tr></thead>
   <tbody>{rows.map(({s,lessons,activities,visual,word})=><tr key={s.id}><th scope="row"><strong>{s.title}</strong><small>{s.id} · {s.topic} · MOE p. {s.sourcePage}</small>{lessons.map(l=><a key={l.id} className="cov-link" href={`#/teach/${l.id}`}>Learn: {l.title}</a>)}{activities.slice(0,2).map(a=><a key={a.id} className="cov-link" href={`#/activity/${a.id}`}>Practise: {a.title}</a>)}</th>
    {tick(lessons.length>0,'Learn lesson')}{tick(lessons.length>0,'Guided practice')}{tick(activities.length>0,'Practice activity')}{tick(visual,'Interactive model')}{tick(word,'Word problems')}{tick(lessons.length>0,'Mastery check')}</tr>)}</tbody></table></div>
  <p className="stage-text">Learn lessons are being written topic by topic, starting with the exemplar units for each year. Where a lesson does not exist yet, the activity still gives generated questions, clues and worked explanations, and a teacher can use the <a href="#/lab">Math Lab</a> models directly.</p>
