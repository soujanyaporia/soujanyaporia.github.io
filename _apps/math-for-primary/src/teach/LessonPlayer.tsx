...
function Player({lesson,mode}:{lesson:Lesson;mode:LessonMode}){
 ...
 return <main className={`teach grade-${lesson.level}`}>
  ...
  <aside className="lesson-purpose"><span>OUR LEARNING GOAL</span><p>{lesson.mission?.goal??lesson.objectives[0]}</p>{index===0&&lesson.mission&&<p className="lesson-route">{lesson.mission.connection}</p>}</aside>
...
function VisualExplanation({stage,done,onDone}:{stage:Extract<Stage,{kind:'explain'}>;done:boolean;onDone:()=>void}){
 ...
 <figure className="visual-tutor-picture teaching-figure" key={`${way}-${at}`}>{f.tool&&<ToolView tool={f.tool}/>}<figcaption>{f.caption??modelCaption(f.tool)}</figcaption></figure>
...
function Worked({stage,done,onDone}:{stage:Extract<Stage,{kind:'worked'}>;done:boolean;onDone:()=>void}){
 ...
 return <Card title={stage.title} read={false}><div className="stage-title"><p className="worked-problem">{stage.problem}</p><SpeakButton text={`${stage.problem}. ${step.text} ${step.because??''} ${step.math??''}`}/></div>{(step.tool??stage.tool)&&<figure className="teaching-figure"><ToolView tool={(step.tool??stage.tool)!}/><figcaption>{step.caption??modelCaption(step.tool??stage.tool)}</figcaption></figure>}<ol className="worked-steps one-at-a-time">{stage.steps.map((s,i)=>i===shown-1&&<li key={i}><p>{s.text}</p>{s.because&&<p className="worked-because">{s.because}</p>}{s.math&&(!s.ask||solved[i])&&<p className="stage-math">{s.math}</p>}{s.ask&&(solved[i]?<p className="stage-feedback good">✓ {s.ask.answer}</p>:<div className="worked-ask"><p><strong>{s.ask.prompt}</strong></p>{s.ask.choices?<div className="notice-options">{s.ask.choices.map(c=><button key={c} className={answers[i]===c?(c===s.ask!.answer?'right':'wrong'):''} onClick={()=>{setAnswers(a=>({...a,[i]:c}));if(c===s.ask!.answer)setSolved(v=>({...v,[i]:true}));}}>{c}</button>)}</div>:<form className="inline-answer" onSubmit={e=>{e.preventDefault();if(isCorrect({answer:s.ask!.answer},answers[i]??''))setSolved(v=>({...v,[i]:true}));else setAnswers(a=>({...a,[i]:''}));}}><input aria-label={s.ask.prompt} inputMode="decimal" value={answers[i]??''} onChange={e=>setAnswers(a=>({...a,[i]:e.target.value}))}/><button className="teach-btn">Check</button></form>}<button className="link-btn" onClick={()=>setSolved(v=>({...v,[i]:true}))}>Show me</button></div>)}</li>)}</ol><div className="visual-tutor-nav"><button className="teach-btn soft" disabled={shown===1} onClick={()=>setShown(n=>n-1)}>← Previous step</button><span>{shown} / {stage.steps.length}</span>{shown<stage.steps.length&&<button className="teach-btn" disabled={waiting} onClick={advance}>Next step →</button>}</div></Card>;
}
