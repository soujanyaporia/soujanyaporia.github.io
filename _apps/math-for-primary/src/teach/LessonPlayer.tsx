...
function StageView({lesson,stage,index,items,attempt,onRecord,onDone,onConfidence,reward}:{lesson:Lesson;stage:Stage;index:number;items:Item[];attempt:LessonAttempt;onRecord:(item:Item,r:ItemRecord)=>void;onDone:()=>void;onConfidence:(c:number)=>void;reward:(t:string)=>void}){
 ...
 switch(stage.kind){
  case 'hook':return <Card title={stage.title} text={stage.text}>{stage.tool&&<figure className="teaching-figure"><ToolView tool={stage.tool}/>{(stage.caption??modelCaption(stage.tool))&&<figcaption>{stage.caption??modelCaption(stage.tool)}</figcaption>}</figure>}<p className="teacher-reassure">{stage.next??'On the next screen, we will stay with this picture and see how the idea works. You do not need to answer yet.'}</p></Card>;
...
function ItemsStage({lesson,stage,index,items,attempt,onRecord,onConfidence}:{lesson:Lesson;stage:Stage;index:number;items:Item[];attempt:LessonAttempt;onRecord:(item:Item,r:ItemRecord)=>void;onConfidence:(c:number)=>void}){
 ...
 return <Card title={title} text={text}><div className="item-dots" role="group" aria-label="Questions in this step">{items.map((it,i)=>{const r=attempt.items[`${index}:${it.key}`];return <button key={it.key} aria-label={`Question ${i+1}${r?', answered':''}`} aria-current={i===view?'step':undefined} className={`${i===view?'now ':''}${r?(r.firstTry?'clean':r.correct?'ok':'shown'):''}`} onClick={()=>setView(i)}/>;})}</div><p className="item-count">Question {Math.min(view,items.length-1)+1} of {items.length}{view>0?' · A new example':''}</p>{questionPurpose(item,stage)&&<p className="question-purpose">{questionPurpose(item,stage)}</p>}
