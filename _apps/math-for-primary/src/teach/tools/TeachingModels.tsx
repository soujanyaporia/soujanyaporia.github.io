...
/** No printed fraction: the learner must read the chosen parts and the whole. */
export function FractionPieces({t}:{t:Extract<Tool,{kind:'fraction-pieces'}>}){
 ...
 return t.shape==='circle'?<svg className="fraction-pieces-circle" viewBox="0 0 220 220" role="img" aria-label={label}>{t.widths.map((w,i)=>{const start=at;at+=w/total*2*Math.PI;return <path key={i} d={`M110 110L${110+96*Math.cos(start)} ${110+96*Math.sin(start)}A96 96 0 ${w/total>.5?1:0} 1 ${110+96*Math.cos(at)} ${110+96*Math.sin(at)}Z`} fill={t.selected.includes(i)?'#536ce7':'#eef0f7'} stroke="white" strokeWidth="3"/>;})}</svg>:<div className="fraction-pieces-strip" role="img" aria-label={label}>{t.widths.map((w,i)=><span key={i} style={{flex:w,background:t.selected.includes(i)?'#536ce7':'#eef0f7'}}/>)}</div>;
}
