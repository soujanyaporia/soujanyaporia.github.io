...
function Bar({t}:Props<'bar'>){
 if(t.compare){const {top,bottom,names}=t.compare,max=Math.max(top??0,bottom??0,1),w=(v:number|null)=>`${((v??max*.6)/max)*100}%`;
  return <div className="tool tool-bar compare" role="img" aria-label={`Comparison model: ${names[0]} ${top??'unknown'}, ${names[1]} ${bottom??'unknown'}`}>{[[names[0],top],[names[1],bottom]].map(([name,v])=><div className="tool-bar-row" key={String(name)}><span className="tool-bar-name">{name}</span><span className="tool-bar-track"><span className="bar-seg" style={{width:w(v as number|null)}}>{v===null?'?':String(v)}</span></span></div>)}{top!==null&&bottom!==null&&<p className="bar-diff">Difference: {Math.abs((top as number)-(bottom as number))}</p>}</div>;}
...
function Groups({t,onChange}:Props<'groups'>){
 ...
 return <div className="tool tool-groups"><div className="plates" role="img" aria-label={`${t.groups} equal groups of ${t.size}: ${total} altogether`}>{Array.from({length:t.groups},(_,g)=><div className="tool-plate" key={g}>{Array.from({length:t.size},(_,i)=><i key={i} className="dot"/>)}</div>)}</div><p className="tool-sum" aria-live="polite">{t.groups&&t.size?`${Array.from({length:t.groups},()=>t.size).join(' + ')} = ${t.groups} × ${t.size} = ${total}`:'Make some equal groups.'}</p>{onChange&&<div className="tool-controls"><Stepper label="Groups" value={t.groups} max={limit} onChange={n=>onChange({...t,groups:n})}/><Stepper label="In each group" value={t.size} max={limit} onChange={n=>onChange({...t,size:n})}/></div>}</div>;
}
function ArrayGrid({t,onChange}:Props<'array'>){
 return <div className="tool tool-array"><div className="tool-array-grid" role="img" aria-label={`Array of ${t.rows} rows and ${t.cols} columns: ${t.rows*t.cols} dots`} style={{width:`${t.cols*30+16}px`,gridTemplateColumns:`repeat(${t.cols},minmax(0,1fr))`}}>{Array.from({length:t.rows*t.cols},(_,i)=><i key={i} className="dot"/>)}</div><p className="tool-sum" aria-live="polite">{t.rows} rows of {t.cols} = {t.rows} × {t.cols} = {t.rows*t.cols}</p>{onChange&&<div className="tool-controls"><Stepper label="Rows" value={t.rows} max={10} min={1} onChange={n=>onChange({...t,rows:n})}/><Stepper label="Columns" value={t.cols} max={10} min={1} onChange={n=>onChange({...t,cols:n})}/><button type="button" onClick={()=>onChange({...t,rows:t.cols,cols:t.rows})}>Turn it a quarter turn</button></div>}</div>;
}
