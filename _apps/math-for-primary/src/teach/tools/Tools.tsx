...
function Ratio({t,onChange}:Props<'ratio'>){
 ...
 return <div className="tool tool-ratio" role={onChange?'group':'img'} aria-label={`Ratio ${t.names.join(' to ')} is ${t.units.join(' : ')}${t.unitValue!==null?`; one unit is ${t.unitValue}`:''}`}>{t.names.map((name,i)=><div className="ratio-row" key={name}><span className="ratio-name">{name}</span><span className="ratio-units">{Array.from({length:t.units[i]},(_,k)=><span key={k} className="ratio-unit" style={{background:colours[i%colours.length]}}>{t.unitValue!==null?fmt(t.unitValue):''}</span>)}</span>{!t.hideValue&&t.unitValue!==null&&<span className="ratio-amount">{fmt(t.units[i]*t.unitValue)}</span>}{onChange&&<Stepper label={`${name} units`} value={t.units[i]} min={1} max={9} onChange={n=>onChange({...t,units:t.units.map((u,j)=>j===i?n:u)})}/>}</div>)}{!t.hideValue&&<p className="tool-sum" aria-live="polite">{t.names.length===1?`${total} equal units`:`${t.names.join(' : ')} = ${t.units.join(' : ')}`}{t.total!=null?` · ${total} units = ${fmt(t.total)}${t.unitValue!==null?` · 1 unit = ${fmt(t.unitValue)}`:''}`:''}</p>}{t.hideValue&&t.total!=null&&<p>Total amount: {t.total}</p>}</div>;
}
