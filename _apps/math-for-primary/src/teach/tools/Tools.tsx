...
import {TakeAway,TrianglePair,FractionPieces} from './TeachingModels';
...
function Bond({t,onChange}:Props<'bond'>){
 ...
 const part=(which:'a'|'b',n:number)=><div className={`bond-part bond-side-${which}${t.covered===which?' bond-covered':''}`} data-drop={which}><small>{t.covered===which?'taken away':t.covered?'stays':'part'}</small><strong>{show(which,n)}</strong>{t.hide!==which&&<div className="bond-dots">{Array.from({length:n},(_,i)=>live?<button type="button" key={i} className="dot" aria-label={`Move a counter from ${which==='a'?'left to right':'right to left'}`} style={drag?.from===which&&i===n-1?{transform:`translate(${drag.dx}px,${drag.dy}px)`}:undefined} {...(i===n-1?handlers(which):{})} onClick={()=>{if(!moved.current)move(which);}}/>:<i key={i} className="dot"/>)}</div>}</div>;
 return <div className="tool tool-bond" role={live?'group':'img'} aria-label={`Number bond: whole ${show('whole',t.whole)}, parts ${show('a',t.parts[0])} and ${show('b',t.parts[1])}${t.covered?`; the ${t.covered==='a'?'left':'right'} part is taken away`:''}`}><div className="bond-whole"><strong>{show('whole',t.whole)}</strong><small>whole</small></div><svg className="bond-lines" viewBox="0 0 200 40" aria-hidden="true"><line x1="100" y1="0" x2="45" y2="40"/><line x1="100" y1="0" x2="155" y2="40"/></svg><div className="bond-parts">{part('a',t.parts[0])}{part('b',t.parts[1])}</div>{live&&<><div className="tool-controls"><button type="button" disabled={t.parts[1]===0} onClick={()=>move('b')}>Move 1 left ←</button><button type="button" disabled={t.parts[0]===0} onClick={()=>move('a')}>Move 1 right →</button></div><p className="tool-tip">Use the buttons, or drag a counter between the parts. The whole stays the same.</p></>}</div>;
}
function Bar({t}:Props<'bar'>){
 if(t.compare){const {top,bottom,names}=t.compare,max=Math.max(top??0,bottom??0,1),w=(v:number|null)=>`${((v??max*.6)/max)*100}%`;
  return <div className="tool tool-bar compare" role="img" aria-label={`Comparison model: ${names[0]} ${top??'unknown'}, ${names[1]} ${bottom??'unknown'}`}>{[[names[0],top],[names[1],bottom]].map(([name,v])=><div className="tool-bar-row" key={String(name)}><span className="tool-bar-name">{name}</span><span className="tool-bar-track"><span className="bar-seg" style={{width:w(v as number|null)}}>{v===null?'?':String(v)}</span></span></div>)}{!t.hideValue&&top!==null&&bottom!==null&&<p className="bar-diff">Difference: {Math.abs((top as number)-(bottom as number))}</p>}</div>;}
...
function NumberLine({t,onChange}:Props<'line'>){
 ...
 return <div className="tool tool-line"><svg viewBox={`0 0 ${W} 96`} role="img" aria-label={`Number line from ${t.min} to ${t.max}, starting at ${t.start}${t.jumps.length?`, jumps ${t.jumps.join(', ')}, ending at ${at}`:''}`}><line className="tool-nl-axis" x1={x0} x2={x1} y1="70" y2="70"/>{Array.from({length:ticks+1},(_,i)=>{const v=t.min+i*step,major=ticks<=20||i%5===0;return <g key={i}><line className="tool-nl-tick" x1={pos(v)} x2={pos(v)} y1={major?62:65} y2="78"/>{major&&<text x={pos(v)} y="92" textAnchor="middle">{fmt(v)}</text>}</g>;})}{arcs.map((a,i)=><g key={i}><path className="tool-nl-jump" d={`M${pos(a.from)} 62 Q${(pos(a.from)+pos(a.to))/2} ${62-Math.min(46,10+Math.abs(pos(a.to)-pos(a.from))/3)} ${pos(a.to)} 62`}/><text className="tool-nl-jump-label" x={(pos(a.from)+pos(a.to))/2} y={Math.max(12,40-Math.abs(pos(a.to)-pos(a.from))/6)} textAnchor="middle">{a.j>0?`+${fmt(a.j)}`:fmt(a.j)}</text></g>)}<circle className="tool-nl-start" cx={pos(t.start)} cy="70" r="6"/>{t.mark!=null&&<circle className="tool-nl-mark" cx={pos(t.mark)} cy="70" r="7"/>}</svg>{onChange&&<div className="tool-controls">{[-10,-1,1,10].filter(j=>Math.abs(j)<=t.max-t.min).map(j=><button type="button" key={j} disabled={at+j<t.min||at+j>t.max} onClick={()=>onChange({...t,jumps:[...t.jumps,j]})}>{j>0?`Jump +${j}`:`Jump ${j}`}</button>)}<button type="button" disabled={!t.jumps.length} onClick={()=>onChange({...t,jumps:t.jumps.slice(0,-1)})}>Undo</button><output aria-live="polite">At {fmt(at)}</output></div>}</div>;
}
function Groups({t,onChange}:Props<'groups'>){
 const limit=t.limit??10,total=t.groups*t.size;
 return <div className="tool tool-groups"><div className="plates" role="img" aria-label={`${t.groups} equal groups of ${t.size}${t.hideValue?'':`: ${total} altogether`}`}>{Array.from({length:t.groups},(_,g)=><div className="tool-plate" key={g}>{Array.from({length:t.size},(_,i)=><i key={i} className="dot"/>)}</div>)}</div>{!t.hideValue&&<p className="tool-sum" aria-live="polite">{t.groups&&t.size?`${Array.from({length:t.groups},()=>t.size).join(' + ')} = ${t.groups} × ${t.size} = ${total}`:'Make some equal groups.'}</p>}{onChange&&<div className="tool-controls"><Stepper label="Groups" value={t.groups} max={limit} onChange={n=>onChange({...t,groups:n})}/><Stepper label="In each group" value={t.size} max={limit} onChange={n=>onChange({...t,size:n})}/></div>}</div>;
}
function ArrayGrid({t,onChange}:Props<'array'>){
 return <div className="tool tool-array"><div className="tool-array-grid" role="img" aria-label={`Array of ${t.rows} rows and ${t.cols} columns${t.hideValue?'':`: ${t.rows*t.cols} dots`}`} style={{width:`${t.cols*30+16}px`,gridTemplateColumns:`repeat(${t.cols},minmax(0,1fr))`}}>{Array.from({length:t.rows*t.cols},(_,i)=><i key={i} className="dot"/>)}</div>{!t.hideValue&&<p className="tool-sum" aria-live="polite">{t.rows} rows of {t.cols} = {t.rows} × {t.cols} = {t.rows*t.cols}</p>}{onChange&&<div className="tool-controls"><Stepper label="Rows" value={t.rows} max={10} min={1} onChange={n=>onChange({...t,rows:n})}/><Stepper label="Columns" value={t.cols} max={10} min={1} onChange={n=>onChange({...t,cols:n})}/><button type="button" onClick={()=>onChange({...t,rows:t.cols,cols:t.rows})}>Turn it a quarter turn</button></div>}</div>;
}
function Share({t,onChange}:Props<'share'>){
 ...
 return <div className="tool tool-share"><div className="pile" aria-label={`${left} left to ${t.mode==='share'?'share':'put in groups'}`}>{Array.from({length:left},(_,i)=>live&&t.mode==='share'&&i===left-1?<button type="button" key={i} className="token" aria-label="Drag this to a plate" {...handlers('pile')}/>:<i key={i} className="token"/>)}{!left&&<span className="pile-empty">None left</span>}</div>
  <div className="plates">{(t.mode==='share'?Array.from({length:t.people},(_,i)=>i):boxes.map((_,i)=>i)).map(i=><div key={i} className="tool-plate" data-drop={`plate-${i}`}>{live&&t.mode==='share'?<button type="button" className="plate-add" onClick={()=>{if(!moved.current)give(i);}} aria-label={`Give one to friend ${i+1} (has ${t.given[i]})`}>{Array.from({length:t.given[i]},(_,k)=><i key={k} className="token"/>)}</button>:Array.from({length:t.given[i]??0},(_,k)=><i key={k} className="token"/>)}<small>{t.mode==='share'?`Friend ${i+1}${t.hideValue?'':`: ${t.given[i]}`}`:`Group ${i+1}${t.hideValue?'':`: ${t.given[i]}`}`}</small></div>)}</div>
...
function FractionWall({t,onChange}:Props<'fractions'>){
 const W=420,H=80,gap=14,pairs=equalRows(t),set=(row:number,k:number)=>onChange?.({...t,shaded:t.shaded.map((s,i)=>i===row?(s===k?k-1:k):s)});
 const matchX=pairs.length?(t.shaded[pairs[0][0]]/t.denominators[pairs[0][0]])*W:null;
 return <div className={`tool tool-fractions${t.hideValue?' fraction-reading':''}`}><svg viewBox={`0 0 ${W+90} ${t.denominators.length*(H+gap)+8}`} role={onChange?'group':'img'} aria-label={t.denominators.map((d,i)=>`${t.shaded[i]} of ${d} equal parts shaded`).join('; ')}>{t.denominators.map((d,row)=><g key={row} transform={`translate(0 ${row*(H+gap)+4})`}>{stripParts(W,d).map((p,k)=>{const on=k<t.shaded[row];return <g key={k}>{onChange?<rect className={`fw-part${on?' on':''}`} x={p.x} y="0" width={p.width} height={H} role="button" tabIndex={0} aria-label={`Shade ${k+1} of ${d}`} onClick={()=>set(row,k+1)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();set(row,k+1);}}}/>:<rect className={`fw-part${on?' on':''}`} x={p.x} y="0" width={p.width} height={H}/>}{!t.hideValue&&d<=12&&<text className="fw-label" x={p.x+p.width/2} y={H/2+5} textAnchor="middle">{d===1?'1':`1/${d}`}</text>}</g>;})}{!t.hideValue&&<text className="fw-value" x={W+14} y={H/2+6}>{t.shaded[row]}/{d}</text>}</g>)}{matchX!==null&&<line className="fw-match" x1={matchX} x2={matchX} y1="0" y2={t.denominators.length*(H+gap)}/>}</svg>{!t.hideValue&&pairs.length>0&&<p className="tool-sum fw-equal" aria-live="polite">{pairs.map(([i,j])=>`${t.shaded[i]}/${t.denominators[i]} = ${t.shaded[j]}/${t.denominators[j]}`).join('  ·  ')}: the same length, so the same amount.</p>}</div>;
}
...
function PercentBar({t,onChange}:Props<'percent'>){
 ...
 return <div className="tool tool-percent"><div className="pbar" role="img" aria-label={`${t.percent}% of a whole of ${show(t.whole)}${t.hideValue?'':` is ${show(value)}`}`}>{Array.from({length:segs},(_,i)=><span key={i} className={(i+1)*step<=t.percent?'on':''}></span>)}</div><p className="pbar-key">Each section: {step}%{!t.hideValue&&` = ${show(t.whole*step/100)}`}</p><div className="pbar-scale" aria-hidden="true"><span>0%</span><span>100% = {show(t.whole)}</span></div>{!t.hideValue&&<p className="tool-sum" aria-live="polite"><strong>{t.percent}%</strong> of {show(t.whole)} = {show(value)}</p>}{onChange&&<div className="tool-controls"><button type="button" disabled={t.percent<=0} onClick={()=>onChange({...t,percent:t.percent-step})}>− {step}%</button><button type="button" disabled={t.percent>=100} onClick={()=>onChange({...t,percent:t.percent+step})}>+ {step}%</button></div>}</div>;
}
function Ratio({t,onChange}:Props<'ratio'>){
 const total=ratioTotal(t),colours=t.colours??['#5b8fd6','#e2a54b','#6fa676'];
 return <div className="tool tool-ratio" role={onChange?'group':'img'} aria-label={`Ratio ${t.names.join(' to ')} is ${t.units.join(' : ')}${t.unitValue!==null?`; one unit is ${t.unitValue}`:''}`}>{t.names.map((name,i)=><div className="ratio-row" key={name}><span className="ratio-name">{name}</span><span className="ratio-units" style={{gridTemplateColumns:`repeat(${Math.max(...t.units)},minmax(0,1fr))`,maxWidth:`${Math.max(...t.units)*55-3}px`}}>{Array.from({length:t.units[i]},(_,k)=><span key={k} className="ratio-unit" style={{background:colours[i%colours.length]}}>{t.unitValue!==null?fmt(t.unitValue):''}</span>)}</span>{!t.hideValue&&t.unitValue!==null&&<span className="ratio-amount">{fmt(t.units[i]*t.unitValue)} altogether</span>}{onChange&&<Stepper label={`${name} units`} value={t.units[i]} min={1} max={9} onChange={n=>onChange({...t,units:t.units.map((u,j)=>j===i?n:u)})}/>}</div>)}{!t.hideValue&&<p className="tool-sum" aria-live="polite">{t.names.length===1?`${total} equal units`:`${t.names.join(' : ')} = ${t.units.join(' : ')}`}{t.total!=null?` · ${total} units = ${fmt(t.total)}${t.unitValue!==null?` · 1 unit = ${fmt(t.unitValue)}`:''}`:''}</p>}{t.hideValue&&t.total!=null&&<p>Total amount: {t.total}</p>}</div>;
}
