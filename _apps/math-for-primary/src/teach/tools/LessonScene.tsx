...
export function LessonScene({t}:{t:SceneTool}){
 ...
 }else if(t.scene==='shape-join'){
  art=<><rect x="90" y="45" width="340" height="190" fill="#f2f4f8" stroke="#9daaba" strokeWidth="3"/><path d="M90 235V45L430 235Z" fill={BLUE}/>{p>=1&&<path d="M90 45H430V235Z" fill={GOLD}/>}<path d="M90 45L430 235" stroke="white" strokeWidth="4" strokeDasharray={p===2?'8 6':undefined}/>{text(260,270,p===0?'One triangle':p===1?'Two matching triangles':'One complete rectangle','shape')}</>;
 }else if(t.scene==='copy-grid'){
  art=<>{[40,310].map((x,i)=><g key={i}>{Array.from({length:30},(_,j)=><circle key={j} cx={x+(j%6)*28} cy={65+Math.floor(j/6)*28} r="2.5" fill="#b2bccc"/>)}{(i===0||p>=2)?<path d={`M${x+28} 93h84v56h-84Z`} fill="#edf0ff" stroke={BLUE} strokeWidth="4"/>:p===1?<path d={`M${x+28} 93h84`} stroke={BLUE} strokeWidth="4"/>:null}<circle cx={x+28} cy="93" r="6" fill={BLUE}/>{text(x+70,225,i===0?'Original':'Your copy','label')}</g>)}{text(260,275,'Each space is one grid step','note')}</>;
 }else if(t.scene==='angle-name'){
  art=<><path d="M240 50L160 210H420" fill="none" stroke={BLUE} strokeWidth="5"/><path d="M202 210A42 42 0 0 0 179 173" fill="none" stroke={GOLD} strokeWidth="5"/>{p===1&&<circle cx="160" cy="210" r="18" fill="#f9d891" opacity=".6"/>}<circle cx="160" cy="210" r="5" fill={BLUE}/>{text(242,35,'A','a')}{text(142,240,'B','b')}{text(435,217,'C','c')}{text(300,265,p===1?'B is the vertex':'Two rays meet at B','vertex')}</>;
 }else if(t.scene==='money'){art=<>{t.values.map((v,i)=><g key={i}>{p>=3?<rect x={30+i*155} y='75' width='140' height='95' rx='10' fill='#e6edcf' stroke='#8caa72' strokeWidth='3'/>:<circle cx={80+i*115} cy={130} r={42} fill='#f8d997' stroke='#c69445' strokeWidth='4'/>}{text(p>=3?100+i*155:80+i*115,138,p>=3?`$${v}`:`${v}¢`,'coin')}</g>)}</>;}else if(t.scene==='place'){
