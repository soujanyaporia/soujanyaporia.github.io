...
export function fractionItem(seed:number,index:number,facet:Facet='visual',support=true):Item{
 ...
 if(facet==='unfamiliar')item={...item,tool:{kind:'fraction-pieces',widths:Array(d).fill(1),selected:r.shuffle(Array.from({length:d},(_,i)=>i)).slice(0,n).sort((a,b)=>a-b),shape:index%2?'circle':'strip'},rep:'diagram',prompt:index%2?'This time the whole is a circle. What fraction is blue?':'Look at the chosen pieces. What fraction of the whole strip is blue?'};
 if(facet==='missing'){
