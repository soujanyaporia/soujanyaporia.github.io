...
export function LearnIndex(){
 ...
 const card=(l:Lesson)=>{const saved=attempts[l.id]?.revision===l.revision?attempts[l.id]:undefined,inLesson=saved?.mode==='learn'&&saved.completedAt===null;const state=lessonState(progress,l,inLesson?saved:undefined),activity=activityById(l.activityId),r=reviewDue(progress,l);return <article key={l.id} className={`lesson-card state-${state}`}><div className="lesson-card-top"><span className="lesson-state">{STATE_LABEL[state]}</span><span className="lesson-minutes">about {l.minutes} min</span></div><h3>{l.title}</h3><p className="lesson-preview"><span>Start with</span>{lessonPreview(l)}</p><div className="lesson-actions"><a className="teach-btn" href={`#/teach/${l.id}`}>{state==='started'?'Continue learning':state==='new'?'Learn':'Learn again'}</a>{activity&&<a className="lesson-secondary" href={`#/activity/${activity.id}`}>Practise</a>}<details className="lesson-extra"><summary>More options</summary><div><a className="lesson-secondary" href={`#/teach/${l.id}/challenge`}>{saved?.mode==='challenge'&&saved.completedAt===null&&Object.keys(saved.items).length?'Continue challenge':'Challenge'}</a>{r.due&&<a className="lesson-secondary" href={`#/teach/${l.id}/review`}>Review due</a>}<a className="lesson-secondary" href={`#/guide/${l.id}`}>Adult guide</a></div></details></div></article>;};
 return <main className={`teach learn-index grade-${level}`}><header className="learn-head"><p className="teach-eyebrow">Learn a new idea</p><h1>{level<=2?'Let’s find out how maths works.':'Make sense of a new idea.'}</h1><p className="stage-text">Choose Learn for pictures and worked steps. You can ask for help whenever you get stuck.</p></header>
