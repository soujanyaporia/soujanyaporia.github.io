...
export function SchoolNav(){
 ...
 return <nav className="school-nav" aria-label="School navigation"><a className="school-wordmark" href="#/">✦ <span>Maths for SG Primary Schools</span></a><div><a href="#/curriculum">Curriculum</a><a href={staff?'#/school':'#/school-start'}>{staff?'My school':'School setup'}</a><a className="account-link" href="#/account">{user?user.name:'School sign in'} <span aria-hidden="true">↗</span></a></div>
