...
interface AccountApi {register:(schoolName:string,name:string,email:string,password:string)=>Promise<void>;classes:SchoolClass[];user:SchoolUser|null;token:string|null;loading:boolean;error:string;api:(path:string,method?:string,body?:unknown)=>Promise<any>;login:(schoolCode:string,username:string,password:string)=>Promise<void>;logout:()=>Promise<void>;refresh:()=>Promise<void>}
...
export function AccountProvider({children}:{children:ReactNode}){
 ...
 const register=useCallback(async(schoolName:string,name:string,email:string,password:string)=>{const r=await apiRequest('/register-school',null,'POST',{schoolName,name,email,password});saveToken(r.token);setUser(r.user);setToken(r.token);setError('');},[]);
 ...
 const value=useMemo(()=>({register,classes,user,token,loading,error,api,login,logout,refresh}),[register,classes,user,token,loading,error,api,login,logout,refresh]);
